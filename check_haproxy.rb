...
op = OptionParser.new do |opts|
  ...
  opts.on("-u", "--url URL", "Statistics URL to check (e.g. http://demo.1wt.eu/)") do |v|
    options.url = v
  ...
  opts.on("-w", "--warning [WARNING]", "Threshold for number of sessions as a percentage of the limit") do |v|
    options.warning = v
  end

  opts.on("-c", "--critical [CRITICAL]", "Threshold for number of sessions as a percentage of the limit") do |v|
    options.critical = v
  end
  ...
  f.each do |line|
    ...
    if %w(FRONTEND BACKEND).include? row['svname']
      if options.critical && row['scur'].to_i * 100 >= options.critical.to_i * row['slim'].to_i
        @errors << sprintf("%s has too many sessions (%s/%s) on %s proxy", row['svname'], row['scur'], row['slim'], row['pxname'])
        exit_code = CRITICAL
